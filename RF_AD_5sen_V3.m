...
%Se consideran los sensores 4, 6, 7, 11 y 12 en VRi
VR1 = [  V1(:,4,:)   V1(:,6,:)   V1(:,7,:)   V1(:,11,:)   V1(:,12,:)];
VR2 = [  V2(:,4,:)  V2(:,6,:)   V2(:,7,:)  V2(:,11,:)   V2(:,12,:)];
VR3 = [  V3(:,4,:)    V3(:,6,:)   V3(:,7,:)   V3(:,11,:)   V3(:,12,:)];
VR4 = [  V4(:,4,:)   V4(:,6,:)   V4(:,7,:)  V4(:,11,:)   V4(:,12,:)];
VR5 = [  V5(:,4,:)  V5(:,6,:)   V5(:,7,:)    V5(:,11,:)   V5(:,12,:)];
VR6 = [  V6(:,4,:)   V6(:,6,:)   V6(:,7,:)    V6(:,11,:)   V6(:,12,:)];
...
%     V6de_a = aumento_datos(V6de,c1,c2,np,na);
    ...
    for nnn = 1:10
        %ENTRENAMIENTO
%        Mdl = fitcecoc(E,S)
%        t = templateTree('AlgorithmForCategorical','exact','NumVariablesToSample','all',...
%            'PredictorSelection','interaction-curvature');
        t = templateTree('AlgorithmForCategorical','exact',...
            'PredictorSelection','interaction-curvature');
        %rng(1); % For reproducibility
        %S=categorical(S);
%        Mdl = fitrensemble(E,S,'Method','Bag','NumLearningCycles',20, ...
%            'Learners',t);     %Regresion   
        Mdl = fitcensemble(E,S,'Method','Bag','NumLearningCycles',100, ...
            'Learners',t);     %Clasificacion 
...
ylabel('Accuracy')
xlabel('Train number')
title('RF, con 0, 100, 500 y 2000 datos aumentados');
