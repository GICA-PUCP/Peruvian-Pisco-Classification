%Análisis de datos del proyecto Piscos, se utilizan los datos de Karina
%Se clasifica por variedad de pisco Italia y Quebranta
%Se utilizan los datos de subida.
%Se utilizan 3 sensores.
%Se realiza el aumento de datos (usando un bucle más ordenado)
%Se considera los datos de prueba seleccionados de manera fija
%Variando los parametros de interpolación-extrapolación
...
Vccc=0.02:0.04:4;
for ccc=Vccc,
    %Parámetros para el aumento de datos
    ccc
    c1=0.5 - ccc;
    c2=0.5 + ccc;
    np=500;%AD;
...
%     V6de_a = aumento_datos(V6de,c1,c2,np,na);
    ...
    kk=3;
    V1de_a = aumento_datos2(V1de,kk,c1,c2,np,na);
    V2de_a = aumento_datos2(V2de,kk,c1,c2,np,na);
    V3de_a = aumento_datos2(V3de,kk,c1,c2,np,na);
    V4de_a = aumento_datos2(V4de,kk,c1,c2,np,na);
    V5de_a = aumento_datos2(V5de,kk,c1,c2,np,na);
    V6de_a = aumento_datos2(V6de,kk,c1,c2,np,na);
...
for k=1:length(Vccc)
    if k<(length(Vccc)-10),
        maccuracy(k) = sum(accuracy(k:(k+10)))/11;
    end;
end;
hold on;
plot(Vccc(1:length(maccuracy)),maccuracy)

ylabel('Accuracy')
xlabel('CV parameter')
title('Red Neuronal con 500 datos aumentados vs CV');
