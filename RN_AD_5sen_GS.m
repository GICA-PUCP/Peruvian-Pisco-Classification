...
%Tercera estructura: todos los datos de 5 sensores y 6 clases a la salida
%Se realiza el aumento de datos (usando Gaussian noise and signal stretching)
...
%Se consideran los sensores 4, 6, 7, 11 y 12 en VRi (antes)
%Se consideran los sensores 12, 4, 11, 6 y 7 en VRi (actual)
VR1 = [  V1(:,12,:)   V1(:,4,:)   V1(:,11,:)   V1(:,6,:)   V1(:,7,:)];
VR2 = [  V2(:,12,:)  V2(:,4,:)   V2(:,11,:)  V2(:,6,:)   V2(:,7,:)];
VR3 = [  V3(:,12,:)    V3(:,4,:)   V3(:,11,:)   V3(:,6,:)   V3(:,7,:)];
VR4 = [  V4(:,12,:)   V4(:,4,:)   V4(:,11,:)  V4(:,6,:)   V4(:,7,:)];
VR5 = [  V5(:,12,:)  V5(:,4,:)   V5(:,11,:)    V5(:,6,:)   V5(:,7,:)];
VR6 = [  V6(:,12,:)   V6(:,4,:)   V6(:,11,:)    V6(:,6,:)   V6(:,7,:)];
...
NumM=12;
%DPS = [11  10  6  2  1];  %Datos de prueba seleccionados (usado en el
%paper)
DPS = [1  4  7  6  8]; %datos de prueba, alternativa 2
...
for AD=[0 100 500 2000]
    ...
    V1de_a = aumento_datos_GS(V1de,sigma,alfa,np,na);
    V2de_a = aumento_datos_GS(V2de,sigma,alfa,np,na);
    V3de_a = aumento_datos_GS(V3de,sigma,alfa,np,na);
    V4de_a = aumento_datos_GS(V4de,sigma,alfa,np,na);
    V5de_a = aumento_datos_GS(V5de,sigma,alfa,np,na);
    V6de_a = aumento_datos_GS(V6de,sigma,alfa,np,na);
    ...
    tam =size(V1de_a);
    S = [ones(tam(1),1)  zeros(tam(1),1) zeros(tam(1),1) zeros(tam(1),1) zeros(tam(1),1) zeros(tam(1),1) 
        zeros(tam(1),1)   ones(tam(1),1)  zeros(tam(1),1)   zeros(tam(1),1) zeros(tam(1),1) zeros(tam(1),1) 
        zeros(tam(1),1)   zeros(tam(1),1)   ones(tam(1),1)   zeros(tam(1),1) zeros(tam(1),1) zeros(tam(1),1) 
        zeros(tam(1),1)   zeros(tam(1),1)     zeros(tam(1),1) ones(tam(1),1) zeros(tam(1),1) zeros(tam(1),1) 
        zeros(tam(1),1)   zeros(tam(1),1)  zeros(tam(1),1) zeros(tam(1),1)  ones(tam(1),1)   zeros(tam(1),1) 
        zeros(tam(1),1)   zeros(tam(1),1)   zeros(tam(1),1) zeros(tam(1),1) zeros(tam(1),1) ones(tam(1),1)    ];
    ...
    for nnn = 1:10
        ...
        for j=0:(num_clases-1)
            for i=1:porcion,
                ...
                disp(['       ', num2str(i) ,'              ', num2str(SP(1)*100,'%.0f'),'%           '...
                    , num2str(SP(2)*100,'%.0f'), '%            ', num2str(SP(3)*100,'%.0f'),'%        ', num2str(SP(4)*100,'%.0f'), '%         '...
                    , num2str(SP(5)*100,'%.0f'), '%        ', num2str(SP(6)*100,'%.0f'), '%               '])
                [maximo,ind]=max(SP);
                if ind==(j+1)
                    aciertos=aciertos+1;
                end;
...
bar(accuracy)
hold on
bar(1:10,accuracy(1:10),'r')
bar(11:20,accuracy(11:20),'b')
bar(21:30,accuracy(21:30),'g')
bar(31:40,accuracy(31:40),'c')

prom1=mean(accuracy(1:10));
prom2=mean(accuracy(11:20));
prom3=mean(accuracy(21:30));
prom4=mean(accuracy(31:40));
plot([0.5 10.5],[prom1  prom1],'k-');
plot([10.5 20.5],[prom2  prom2],'k-');
plot([20.5 30.5],[prom3  prom3],'k-');
plot([30.5 40.5],[prom4  prom4],'k-');

ylabel('Accuracy')
xlabel('Train number')
title('Red Neuronal, con 0, 100, 500 y 2000 datos aumentados');
